...
Create a new file in the `src/locales` directory with the locale code of the language you want to add. For example, if you want to add support for the French language, create a file named `fr.ts`.

## 3. Define the language definition

In the new file, define and export as default the language definition using the `LanguageDefinition` type from the `src/language/definition.ts` module. Refer to the existing language definitions for examples.
...
## 4. Add the language to the README

Add the new language to the list of supported languages in the root README file.

## 5. Add tests for the language

Add tests to ensure the library handles them correctly if changes are made in the future. Refer to existing tests for examples.
